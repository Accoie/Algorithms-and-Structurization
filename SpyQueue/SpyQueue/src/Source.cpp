...
void QueueSpy::addToQueue(int val1, int val2, string name) {
    Spy* newSpy = new Spy(val1, val2, name);
    if (last) {
        last->next = newSpy;
        newSpy->prev = last; 
    }
...
void QueueSpy::checkSpy(int &curr_time) {
    ...
    if (first){
        cout << curr_time << " минут: Шпион "<< first->name << " взял бинокль. \n\n";
        ...
        if (first == last && prev_first_deleted == 0) {
            cout << first->queue_time << " минут: Шпион " << first->name << " сдал бинокль и ушел к резиденту.\n\n";
            delete first;
            first = nullptr;
            last = nullptr;
        } else if (first && prev_first_deleted == 0) {
            cout << curr_time << " минут: Шпион " << first->name << " сдал бинокль.\n\n";
            last->next = first;
            Spy* temp = last;
            last = first;
            last->prev = temp;
...
void QueueSpy::printTimeOutSpies(vector<Spy*> vec) {
    for (int i = 0; i != vec.size(); ++i) {
        if (vec[i] == first) {
            cout << vec[i]->queue_time << " минут: Шпион " << vec[i]->name << " сдал бинокль и ушел к резиденту.\n\n";
        } else {
            cout << vec[i]->queue_time << " минут: Шпион " << vec[i]->name << " ушел к резиденту.\n\n";
        }
    }
}
int QueueSpy::removeTimeOutSpies(int curr_time) {
    ...
    /*while (first && (first->queue_time - curr_time <= 0)) {
        std::cout << "Шпион #" << first->name << " ушел к резиденту.\n";
        Spy* temp = first;
        first = first->next;
        delete temp;
        if (!first) {
